...
// In-memory session store for user active carts & order states
const sessions = {};

// Helper: Get or initialize user session
function getSession(phone) {
  if (!sessions[phone]) {
    sessions[phone] = {
      cart: [], // Stores selected menu items
      step: 'IDLE', // Current conversation state
      vendorId: null
    };
  }
...
// Helper: Send interactive buttons (up to 3 buttons)
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    // 1. Initial Main Menu (Greeting or Reset)
    ...
    // 3. Selected a Restaurant -> Fetch Menu Items
    ...
    // 4. Selected a Menu Item -> Add to Cart & Ask to Add More or Checkout
    ...
    // 5. User Wants to Add More Items -> Show Menu Again
    ...
    // 6. User Clicks Checkout -> Ask for Delivery Drop-off Location
    ...
    // 7. User Sends Drop-off Address -> Generate 4-Digit Delivery Code & Confirm Order
    if (session.step === 'AWAITING_LOCATION' && text) {
      ...
      // Send Order Confirmation with 4-Digit Delivery Code
      const confirmationMsg = `🎉 *Order Placed Successfully!*\n\n` +
        ...
        `*Note:* Please share this 4-digit PIN with the delivery rider upon arrival to confirm your package receipt. Thank you for using David Delivery!`;
    ...
    // 9. Package Delivery Service Option
...
app.listen(PORT, () => console.log(`🚀 Server running on port ${PORT}`));.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imdsa3V0ZGt3YnJqcGl1cWNtZ3dlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODk5NzU3MTMsImV4cCI6MjEwNTU1MTcxM30.f2SgZJvP681imm0Qe1fKsATnCk_86z2guwvZWASXoZM';
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
